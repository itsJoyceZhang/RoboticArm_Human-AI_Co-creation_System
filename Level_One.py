...
# set the path to the directory where you want to save the image
...
while True:
    ...
    if is_first_time:
        x, y, width, height = cv2.getWindowImageRect("Targeted Area")
        print("窗口像素尺寸为：{} x {}".format(width, height))
        is_first_time = False
...
cv2.imwrite(os.path.join(save_dir, "captured_image.jpg"), roi)

# Read the image and convert to grayscale
img = cv2.imread("../../input/captured_image.jpg")
...
cv2.imshow('Binary Image for edit', img_clahe)
cv2.waitKey(500)
cv2.destroyAllWindows()
# # 对图像进行边界填充
# borderType = cv2.BORDER_CONSTANT
...
try:
    ...
    # height, width = skeletonImg.shape[:2]
    # resized_ske_img = cv2.resize(skeletonImg, (int(width / 2), int(height / 2)))
    path_feedback = np.zeros_like(binaryImg)####
